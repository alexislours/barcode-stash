...
nonisolated extension DataMatrixEncoder {
    ...
    /// GF(256) tables with polynomial 0x12D
    static let gfLog: [Int] = {
        var log = [Int](repeating: 0, count: 256)
        var pow = 1
        for idx in 0 ..< 255 {
            log[pow] = idx
            pow *= 2
            if pow >= 256 { pow ^= 0x12D }
        }
        return log
    }()

    static let gfAlog: [Int] = {
    ...
    static func createECCBlock(
        codewords: [UInt8],
        numECWords: Int
    ) -> [UInt8] {
        guard let tableIndex = factorSets.firstIndex(
            of: numECWords
        ) else { return [] }
        let poly = factors[tableIndex]

        var ecc = [Int](repeating: 0, count: numECWords)
        for codeword in codewords {
            let mix = ecc[numECWords - 1] ^ Int(codeword)
            for kdx in stride(
                from: numECWords - 1, through: 1, by: -1
            ) {
                if mix != 0, poly[kdx] != 0 {
                    ecc[kdx] = ecc[kdx - 1]
                        ^ gfAlog[
                            (gfLog[mix] + gfLog[poly[kdx]]) % 255
                        ]
                } else {
                    ecc[kdx] = ecc[kdx - 1]
                }
            ...
            if mix != 0, poly[0] != 0 {
                ecc[0] = gfAlog[
                    (gfLog[mix] + gfLog[poly[0]]) % 255
                ]
            } else {
                ecc[0] = 0
            }
        }
        return ecc.reversed().map { UInt8($0) }
    }

    static func encodeECC200(
        data: [UInt8],
        symbolInfo: SymbolInfo
    ) -> [UInt8] {
        let blockCount = symbolInfo.interleavedBlockCount
        var result = data
            + [UInt8](repeating: 0, count: symbolInfo.errorCodewords)

        if blockCount == 1 {
            let ecc = createECCBlock(
                codewords: data,
                numECWords: symbolInfo.errorCodewords
            )
            return data + ecc
        }
        ...
        for block in 0 ..< blockCount {
            var temp: [UInt8] = []
            var dataIdx = block
            while dataIdx < symbolInfo.dataCapacity {
                temp.append(data[dataIdx])
                dataIdx += blockCount
            }
            let errorWords = symbolInfo.errorLengthForBlock(block + 1)
            let ecc = createECCBlock(
                codewords: temp, numECWords: errorWords
            )
            var pos = 0
            var errorIdx = block
            while errorIdx < errorWords * blockCount {
                result[symbolInfo.dataCapacity + errorIdx] = ecc[pos]
                pos += 1
                errorIdx += blockCount
            }
        }
